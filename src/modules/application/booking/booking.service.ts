...
export class BookingService {
  ...
  try {
    // Fetch TimeSlot including availability
    ...
    if (!timeSlot || !timeSlot.availability) {
      return { success: false, message: 'Invalid time_slot_id' };
    }

    const bookingDate = new Date(timeSlot.availability.date).toISOString();
    ...
    const service = await this.prisma.service.findUnique({
      ...
      select: { name: true, price: true, car_wash_station_id: true },
    });
    if (!service) return { success: false, message: 'Invalid service_id' };

    // Calculate total amount
    const totalAmount = await this.calculateTotalAmount(dto.service_id, dto.voucher_code);
    if (!totalAmount.success) return totalAmount;
    ...
    try {
      ...
      const booking = await this.prisma.booking.create({
        data: {
          ...
          car_wash_station: {
            connect: { id: createBookingDto.car_wash_station_id },
          },
          ...
          payment_provider_charge_type:
            createBookingDto.payment_provider_charge_type,
      ...
      return {
        ...
        data: {
          ...
          note: paymentIntent
            ...
            : 'Payment intent creation failed, but booking was created',
        },
      };
    } catch (error) {
      return {
          ...
          switch (userDetails.type) {
            case 'washer':
              ...
              if (userDetails.car_wash_station) {
                whereClause.car_wash_station_id =
                  userDetails.car_wash_station.id;
              } else {
        ...
        for (const booking of bookings) {
          if (booking.car_wash_station.image) {
            booking.car_wash_station['image_url'] = SojebStorage.url(
              appConfig().storageUrl.carWashStation +
                booking.car_wash_station.image,
            );
          }
        ...
        if (userDetails) {
          switch (userDetails.type) {
            case 'washer':
              ...
              if (washerStation) {
                whereClause.car_wash_station_id =
                  washerStation.car_wash_station_id;
              }
              break;
            case 'user':
              // Regular users can only see their own bookings
            ...
            case 'washer':
              ...
              if (
                ...
                return {
                  success: false,
                  message:
                    'Access denied. You can only view bookings for your car wash station.',
                };
              }
            case 'user':
  ...
  async update(id: string, updateBookingDto: UpdateBookingDto, userId: string) {
    try {
          ...
          switch (userDetails.type) {
            case 'washer':
              ...
              if (
                washerStation &&
                washerStation.car_wash_station_id ===
                  existingBooking.car_wash_station_id
              ) {
                ...
                return {
                  success: false,
                  message:
                    'Access denied. You can only update bookings for your car wash station.',
                };
              }
            case 'user':
      ...
      if (updateBookingDto.bookingDate) {
        try {
          updateBookingDto.bookingDate = new Date(
            ...
            case 'washer':
              ...
              if (
                ...
                return {
                  success: false,
                  message:
                    'Access denied. You can only delete bookings for your car wash station.',
                };
              }
            case 'user':
    ...
    try {
      ...
      if (existingBookings.length > 0) {
        return {
          success: false,
          message:
            'This time slot is already booked for the requested date. Please choose another time or date.',
          data: {
      ...
      if (!timeSlot) {
        return {
          success: false,
          message:
            'Time slot not found or does not belong to the specified car wash station.',
        };
      }
